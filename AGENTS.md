...
- **Single-advert breaks are skipped**: The pipeline detects ad breaks with only 1 advert via the metadata JSON and skips OCR detection and clipping entirely. Details are logged to `outputs/single_advert_breaks.csv` (appended each run).
